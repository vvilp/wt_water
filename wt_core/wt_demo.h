...
{
	wt_r32 r = 3;
    for (wt_r32 i = 0; i <= w->width +10 ; i += 2 * r)
    {
...
static void wt_circle_pyramid(wt_world *w)
{
    ...
    for (wt_r32 i = 0 ; i < num ; i++)
    {
        for (wt_r32 j = 0 ; j <= i ; j++)
        {
            ...
            wt_r32 y = w->width - r   - i * 2 * r ;
            x += j * 2 * r;
            wt_body *b3 = wt_create_body0(100, wt_v(x, y), 10.0);
            b3->fric = 0.8 ;
            b3->restitution = 0.2;
            b3->ael.y = -40.8;
            wt_circle *c3 = wt_create_cir(b3, r);
...
static void wt_generate_fluid_partical(wt_world *w,float x, float y, float r)
{
    ...
    wt_body *b = wt_create_body0(10, wt_v(x, y), 0);
    b->ael = wt_v(0, -10);
...
{
    wt_body *b3 = wt_create_body0(20, wt_v(x, y), 10.0);
    b3->fric = 0.8 ;
    b3->restitution = 0.2;
    b3->ael.y = -50.8;
    wt_circle *c3 = wt_create_cir(b3, r);
