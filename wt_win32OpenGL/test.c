...
void wt_cir_wall(wt_world *w)
{
    ...
    {
        wt_body *b3 = wt_create_body0(WT_MAX_R32, wt_v(i, w->width / 50), 10.0);
        b3->fric = 0.8 ;
        b3->restitution = 0.2;
        wt_circle *c3 = wt_create_cir(b3, w->width / 50);
        wt_shape *s3 = wt_create_shape(c3, WT_CIR);
        wt_world_add_shape(w, s3);
    }
}
...
void wt_generate_fluid_partical(float x, float y, float r)
{
    ...
    wt_body *b = wt_create_body0(20, wt_v(x, y), 0);
    b->ael = wt_v(0, -5);
    wt_pvf_partical *pvf_p = wt_create_pvf_partical(b);
    wt_pvf_add_partical(w_world->fluid, pvf_p);
}

void wt_generate_circle(float x, float y, float r)
{
    wt_body *b3 = wt_create_body0(500, wt_v(x, y), 10.0);
    b3->fric = 0.8 ;
    b3->restitution = 0.2;
...
void Mouse(int button, int state, int x, int y)
{
    ...
    if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN)
    {
        ...
        wt_generate_circle(real_x, real_y, 2);
